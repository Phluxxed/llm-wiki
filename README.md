...
For a pending candidate edit, use `.venv/bin/python3 scripts/eval.py --gate --changed-since HEAD` (plus an explicit `--judge` when the wiki is owner-locked). The evaluator includes both tracked and untracked wiki pages changed from that Git ref. Structural validity still covers the whole wiki; grounding and redundancy cover only changed pages; contradiction and near-duplicate checks use the whole wiki as context but only candidate-related findings fail the gate. Scoped runs fail loudly for an invalid ref or an empty page slice and record the base ref and exact page list in `.eval/`. Omit `--changed-since` for a deliberate whole-wiki audit.
